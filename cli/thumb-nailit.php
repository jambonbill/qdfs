...
class thumbnailit{
	...
	$type=0;
	if (preg_match("/jpe?g$/",$old_name)){$type=1; $working_name=imagecreatefromjpeg($old_name);}
	if (preg_match("/png$/",$old_name)){$type=2; $working_name=imagecreatefrompng($old_name);}
	if (preg_match("/gif$/",$old_name)){$type=3; $working_name=imagecreatefromgif($old_name);}

	//echo "<LI>count:".count($img_file)."<BR>";
	//print_r($img_file);
	//echo "<LI>\$type=$type<BR>";
	...
	if(!file_exists($new_name)){ // if the file of a given name doesn't exist
		...
		if ($report == 1 ){
		print "Image <b>$new_name</b> already exists, and was <b>not</b> overwritten.<hr>";
		}
	

	}
		if ($report == 1 ){
		print "Image <b>$old_name</b> is already within the given $wanted_size pixels.<hr>";
		}
